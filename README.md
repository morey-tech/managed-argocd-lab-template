...
## 1. Lab Scenario
You are a Kubernetes cluster administrator who works in an organization where developers deploy containerized applications using Helm charts.
...
You can skip this section if you brought a Kubernetes cluster to the lab.

Otherwise, you will create one on your local machine using `kind`.
...
With auto-sync enabled on the `guestbook` Application, a change made in Git will be applied automatically to the cluster. You will demonstrate this by updating the number of replicas for the `guestbook` deployment.
